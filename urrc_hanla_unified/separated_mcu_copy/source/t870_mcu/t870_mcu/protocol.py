...
def speed_mps_to_stage(value: float, stage_speeds=(0.229, 0.455, 0.70)):
    """Convert a signed physical speed command to the closest firmware stage."""
    if not math.isfinite(value):
        return None
    speeds = tuple(float(v) for v in stage_speeds)
    if len(speeds) != 3 or any(v <= 0.0 for v in speeds):
        raise ValueError("stage_speeds must contain three positive values")
    if abs(value) < 1e-4:
        return 0
    index = min(range(3), key=lambda i: abs(abs(value) - speeds[i]))
    return (index + 1) * (1 if value > 0 else -1)
