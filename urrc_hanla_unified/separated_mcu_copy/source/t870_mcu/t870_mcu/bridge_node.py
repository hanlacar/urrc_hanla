...
class McuBridge(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("input_drive_is_speed_mps", False)
        self.declare_parameter("input_wheel_is_float", False)
        self.declare_parameter("speed_stage_mps", [0.229, 0.455, 0.70])
        self.declare_parameter("slope_hold_topic", "")
        self.declare_parameter("slope_hold_pwm", 40)
        ...
        self.input_drive_is_speed = bool(gp("input_drive_is_speed_mps"))
        self.input_wheel_is_float = bool(gp("input_wheel_is_float"))
        self.speed_stage_mps = tuple(float(v) for v in gp("speed_stage_mps"))
        self.slope_hold_pwm = int(gp("slope_hold_pwm"))
        ...
        self.slope_hold = False
        self._slope_hold_sent = False
        ...
        wheel_type = Float32 if self.input_wheel_is_float else Int32
        self.sub_wheel = self.create_subscription(
            wheel_type, self.in_wheel_topic, self.cb_wheel, 10)
        ...
        hold_topic = str(self.get_parameter("slope_hold_topic").value)
        self.sub_slope_hold = (self.create_subscription(
            Bool, hold_topic, self.cb_slope_hold, 10) if hold_topic else None)

        #  타입·QoS 불일치는 에러 없이 메시지를 삼킨다. 주기적으로 확인한다.
        self._sub_specs = [
            (self.in_drive_topic, "std_msgs/msg/Float32", "구동 명령"),
            (self.in_wheel_topic, "std_msgs/msg/Float32" if self.input_wheel_is_float
             else "std_msgs/msg/Int32", "조향 명령"),
    ...
    def cb_drive(self, msg: Float32):
        stage = (speed_mps_to_stage(float(msg.data), self.speed_stage_mps)
                 if self.input_drive_is_speed else parse_drive_stage(float(msg.data)))
        now = time.monotonic()
        if stage is None:
            self.get_logger().error(
                "invalid drive=%r (%s); 거부하고 정지."
    ...
    def cb_slope_hold(self, msg: Bool):
        requested = bool(msg.data)
        if requested != self.slope_hold:
            self._slope_hold_sent = False
        self.slope_hold = requested
        self.wheel_timeout_warned = False
    ...
    def tx_tick(self):
        ...
        if self.slope_hold and not self.estop_latched:
            if not self._slope_hold_sent:
                self._slope_hold_sent = self.send_line("H%d" % self.slope_hold_pwm)
            return
        if self._slope_hold_sent:
            self.send_line("H0")
            self._slope_hold_sent = False
