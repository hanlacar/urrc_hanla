...
from .stop_detection import update_box_confirmation
...
class StopLineGroundNode(Node):
    def __init__(self):
        ...
        defaults={"max_detection_age_sec":.35,"confirmation_frames":3,"camera_x_m":.41,"camera_y_m":0.,"camera_z_m":.85,"camera_mount_roll_deg":0.,"camera_mount_pitch_deg":-5.,"camera_mount_yaw_deg":0.,"box_vertical_ratio":.5,"maximum_distance_m":30.}
        for name,value in defaults.items():self.declare_parameter(name,value)
        self.info=None;self.box=None;self.box_time=None;self.stop_tracker=None
    ...
    def on_detections(self,msg):
        try:
            ...
            if stops:
                candidate=max(stops,key=lambda i:float(i.get("confidence",0.)))["xyxy"]
                confirmed,self.stop_tracker=update_box_confirmation(
                    candidate,self.stop_tracker,self.p("confirmation_frames"),0.3)
                self.box=candidate if confirmed else None
            else:self.stop_tracker=None;self.box=None
            self.box_time=time.monotonic()
        except (ValueError,TypeError,KeyError,json.JSONDecodeError):self.stop_tracker=None;self.box=None;self.box_time=time.monotonic()
    def publish_distance(self):
        ...
        # Publish at the timer rate even when detection is invalid so monitors
        # can distinguish a stale topic from a currently unavailable distance.
        self.distance_pub.publish(Float32(
            data=float(distance) if valid else float("nan")))
