...
The default deployment model is `hanla_competition_11class_best.engine`,
exported at fixed 640x480, batch-1 FP16 from
`hanla_competition_11class/weights/best.pt` for the local MX450. It is an
11-class Ultralytics segmentation model. Ultralytics is lazy-imported when the
node loads the model. A detection checkpoint, missing
road mapping, CUDA
