...
constexpr float         ENCODER_CPR         = 436.0f;
...
void requestBrake()
{
  ...
  //   상위가 B 를 BRAKE_PULSE_MS(150ms)보다 짧은 주기로 반복 전송하면
  //   updateBrake() 의 종료 조건 (now - brakeStartMs >= BRAKE_PULSE_MS) 이
  //   영원히 성립하지 않는다. 결과는 역토크 무한 유지 = 후진.
  //
